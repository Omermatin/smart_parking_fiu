...
class ClassScheduleParser {
  static ClassSchedule? getCurrentOrUpcomingClass(
    ...
    // Reuse the list of todays classes generated by getAllTodayClasses to avoid
    // iterating and filtering the JSON multiple times.
    ...
    for (final classSchedule in todayClasses) {
      ...
      if (startTime == null || endTime == null) {
        // Skip any class that doesnt have a valid time range.
        continue;
      }

      // If the current time falls within the class's duration, return it.
      if (now.isAfter(startTime) && now.isBefore(endTime)) {
        return classSchedule;
      }

      // Since todayClasses is sorted by start time, the first class with a start
      // time after now is the next upcoming class.
      if (now.isBefore(startTime)) {
        return classSchedule;
      }
    }

    // No current or upcoming class was found for the rest of the day.
    return null;
  }
  ...
  // New function to get ALL classes for today
  static List<ClassSchedule> getAllTodayClasses(
    ...
    for (var term in terms) {
      ...
      for (var classItem in classes) {
        // Skip classes that are not In Person
        ...
        // Get all meetings that are scheduled for today
        final validMeetings =
            meetings
                .where(
                  (meeting) =>
